...
class WeightedEdge(Edge):
    """
    A weighted edge with two weights
    """
...
class WeightedDigraph(Digraph):
    """
    A weighted directed graph, each edge has two weights.
    """
    def addEdge(self, weightededge):
        ...
        if not(src in self.nodes and dest in self.nodes):
            raise ValueError('Node not in graph')
        self.edges[src].append([dest, (float(total), float(outdoor))]) #{a: [ [b,(2,1)], [c,(3,2)]], b: [[c,(4,2)]], c:[] }
    def childrenOf(self, node):
        temp = []
    ...
    def __str__(self):
        ...
        for key in self.edges:
            for des in self.edges[key]:
                res = '{0}{1}->{2} ({3}, {4})\n'.format(res, key, des[0], des[1][0], des[1][1])
        return res[:-1]
    def distance(self, src, des):
        for data in self.edges[src]:
            if data[0] == des:
                return tuple(data[1])
        raise ValueError('Node not in graph')
