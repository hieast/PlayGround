...
class SimpleVirus(object):
    ...
    def __init__(self, maxBirthProb, clearProb):
        ...
        # TODO
        assert 0 <= maxBirthProb <=1 and 0 <= clearProb <=1
...
class Patient(object):
    ...
    def update(self):
        ...
        for viruse in self.viruses:
            try:
                tempViruses.append((viruse.reproduce(self.popDensity)))
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        if newDrug not in self.prescription:
            ...
            #print 'The list of drugs being administered to a patient is updated'
    ...
    def update(self):
        ...
        #virus particles accordingly
        tempViruses = []
        for viruse in self.getViruses():
            if not viruse.doesClear():
                tempViruses.append(viruse)
        self.viruses = tempViruses[:]
        
        self.popDensity = len(self.getViruses())/float(self.getMaxPop())
...
def simulationWithDrug(numViruses = 100, maxPop = 1000, maxBirthProb = 0.1, 
                       ...
                       mutProb = 0.005, numTrials = 100, times = 300):
    ...
    for trial in range (numTrials):
        ...
        patient.addPrescription('guttagonol')
        for time in range(times/2):
            total[times/2+time] += patient.update()
            resistant[times/2+time] += patient.getResistPop(['guttagonol'])
    for i in range(times):
