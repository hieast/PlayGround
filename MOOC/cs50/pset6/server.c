...
char* indexes(const char* path)
{
    char *path_index = malloc(strlen(path) + strlen("index.html") + 1);
    
    sprintf(path_index, "%s%s", path, "index.php");
    if (access(path_index, F_OK) != -1)
    {
        return path_index;
    }
    sprintf(path_index, "%s%s", path, "index.html");
    if (access(path_index, F_OK) != -1)
    {
        return path_index;
    }
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    unsigned int size = 0;
    unsigned int room = 1;
    BYTE* file_content = malloc(LimitRequestFieldSize * room + 1);
    BYTE buffer2[LimitRequestFieldSize];
    *length = 0;
    size_t len;
    
    while( (len = fread(buffer2, 1, LimitRequestFieldSize, file)) != 0)
    {
        size++;
        if (size > room)
        {
            room *= 2;
            file_content =  realloc(file_content, LimitRequestFieldSize * room + 1);
        }
        //strncat(file_content, buffer, LimitRequestFieldSize);
        bcopy(buffer2, file_content + *length , len);
        *length += len;
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char target[LimitRequestLine * 2 + 1];
    char version[LimitRequestLine + 1];
    strncpy(method, line, strchr(line, ' ') - line);
    method[strchr(line, ' ') - line] = '\0';
    strncpy(target, strchr(line, ' ') + 1, strrchr(line, ' ') - strchr(line, ' ') - 1);
    target[strrchr(line, ' ') - strchr(line, ' ') - 1] = '\0';
    strncpy(version, strrchr(line, ' ') + 1, strchr(line, '\r') - strrchr(line, ' ') - 1);
    version[strchr(line, '\r')  - strrchr(line, ' ') - 1] = '\0';
    char *ques = strchrnul(target, '?');
    ...
    else if (strchr(target, '"') != NULL || strchr(target, ' ') != NULL)
    {
        error(400);
        return false;
    }
    ...
    //printf("%s%s%s%s%s",line, request, version, ques, quote);

    if (*ques == '?'){
        strncpy(abs_path, target, ques - target);
        abs_path[ques - target] = '\0';
        strcpy(query, ques + 1);
    }
    else{
