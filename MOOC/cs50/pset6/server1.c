...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    if (*ques == '?'){
        strncpy(abs_path, request, ques - request);
        abs_path[ques - request] = '\0';
        strncpy(query, ques + 1, version - ques - 2);
        query[version - ques - 2] = '\0';
    }
    else{
        strncpy(abs_path, request, version - request - 1);
        abs_path[ version - request - 1] = '\0';
        query = "";
